...
if sum(strcmp(varargin,'task_2019'))==1
    tmp=varargin(find(strcmp(varargin,'task_2019'))+1);
    task_type=tmp{1,1};
end
    ...
    case {'decision_random'}
        ...
        if(check_cond)
            if(check_cond0)
                ...
                if(myState.action_prob(myState.index)<0.5)
                    myState.action_history(myState.index)=1;
                else
                    myState.action_history(myState.index)=2;
                end
        ...
        if(check_cond)
            if(check_cond0)
                ...
                if task_type == 2019 || task_type == 2020
                    myState.action_history(myState.index)=myMap.data(myMap.trial,7+myState.index);
                else
        ...
        if(size(varargin,2)>=2) % use the arbitrator's Q for decision
            ...
            if(check_cond)
                if(check_cond0)
                    ...
                    % softmax
                    var_exp=exp(myParam.tau*myArbitrator.Q(state_current,:)); % (N_actionx1)
    ...
    case {'decision'}
        ...
        if(check_cond)
            if(check_cond0) % if not in a terminal state
                var_exp=exp(myParam.tau*myState.Q(state_current,:)); % (N_actionx1)
                Prob_action=var_exp/sum(var_exp); % (N_actionx1)
                myState.action_prob(myState.index)=Prob_action(1);
                if(rand<Prob_action(1))
                    myState.action_history(myState.index)=1;
...
%         update_cond0=(myMap.IsTerminal(state_current)~=1);
        if(update_cond)
            ...
            if(myState.SARSA(5)~=0)
                myState.RPE_history(myState.index)=myParam.gamma*myState.Q(myState.SARSA(4),myState.SARSA(5))-...
...
%             disp(myState.SARSA)
            ...
            % Q-update     
            if myState.SARSA(4) == 6 || myState.SARSA(4) ==7 || myState.SARSA(4) ==8 || myState.SARSA(4) == 9
                myState.reward_state(myState.SARSA(4)) = myState.SARSA(3);              
            end
            tmp_sum=0;
...
%             tmp_sum=tmp_sum+myState.T(myState.SARSA(1),myState.SARSA(2),myState.SARSA(4))*...
%                 (myMap.reward(myState.SARSA(4))+max(myState.Q(myState.SARSA(4),:))); % for (s,a,s')
            tmp_sum=tmp_sum+myState.T(myState.SARSA(1),myState.SARSA(2),myState.SARSA(4))*...
                (myState.reward_state(myState.SARSA(4))+max(myState.Q(myState.SARSA(4),:))); % for (s,a,s')
            if tmp_sum > 40
                disp([num2str(myState.SARSA(4)) ' ' num2str(tmp_sum) ' ' num2str(myState.T(myState.SARSA(1),myState.SARSA(2),myState.SARSA(4))) ' ' num2str(myState.reward_state(myState.SARSA(4))) ' ' num2str(max(myState.Q(myState.SARSA(4),:)))])
            end
            for j=array_rest % for the rest
                tmp_sum=tmp_sum+myState.T(myState.SARSA(1),myState.SARSA(2),j)*...
                    (myState.reward_state(j)+max(myState.Q(j,:)));
%                     (myMap.reward(j)+max(myState.Q(j,:)));
                if tmp_sum > 40
                    disp([num2str(j) ' ' num2str(tmp_sum) ' ' num2str(myState.T(myState.SARSA(1),myState.SARSA(2),j)) ' ' num2str(myState.reward_state(j)) ' ' num2str(max(myState.Q(j,:)))])
                end
            end
            myState.Q_old=myState.Q(myState.SARSA(1),myState.SARSA(2));
            myState.Q(myState.SARSA(1),myState.SARSA(2))=tmp_sum;
        end
%         disp(num2str(myMap.reward))
%         disp(num2str(squeeze(myState.T(1,:,:))))
        % note: if s' is the terminal state ... no Q_sarsa(s',a') term in the sarsa update
        % SARSA matrix reset
%         myState.SARSA=[0 0 0 0 0 0];
