...
%     % 2. model 2 (m2) - THIS WILL NOT BE USED IN THIS VERSION - Bayesian_arbitration_v4.m
    ...
    % (0) backup old values
        ...
        ind_update=length(ind_neg)+1;
        %%
        if -1*myArbitrator.m2_thr_PE > myState2.RPE_history(myState2.index); ind_update = 0; disp('neg m2 hierarchy in Bayesian_Arbitration_V4.m'); end
        %%
        % (2) update the current column(=1) in PE_history
...
% input0=myArbitrator.m2_inv_Fano;    input=myArbitrator.m2_wgt'*myArbitrator.m2_inv_Fano; % added [SW MAR 8]
 if myArbitrator.opt_m2==0
    ...
    input2=myArbitrator.m2_wgt'*myArbitrator.m2_inv_Fano; % added [SW MAR 8]
 else
     input0=1;
     input2=[0 1.0 0]*myArbitrator.m2_inv_Fano;
 end
